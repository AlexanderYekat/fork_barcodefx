...
import static site.barsukov.barcodefx.utils.ScannerUtils.fixLine;

@Slf4j
@Component
@RequiredArgsConstructor
@FxmlView("/fxml/scanner.fxml")
public class ScannerController implements Initializable {
    ...
    public void saveFile(ActionEvent actionEvent) throws IOException {
        ...
        if (file != null) {
            try {
                saveTextToFile(file);
                saveFileButton.getScene().setUserData(file);
                closeComPort();
                GoodCategory goodCategory = category.getSelectionModel().getSelectedItem();
                statisticService.logAction("SCAN_ITEMS_IN_FILE", textList.getItems().size(), goodCategory);
...
//        FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/fxml/picking_order_add.fxml"));
    ...
    private void addLine(String text) {
        GoodCategory goodCategory = category.getSelectionModel().getSelectedItem();
        text = fixLine(text, goodCategory);
    ...
    private void setDefaults() {
        speed.getSelectionModel().select(propService.getProp(SCANNER_CONTROLLER_SPEED));
        stopBit.getSelectionModel().select(propService.getProp(SCANNER_CONTROLLER_STOP_BIT));
        bit.getSelectionModel().select(propService.getProp(SCANNER_CONTROLLER_BIT));
        port.getSelectionModel().select(propService.getProp(SCANNER_CONTROLLER_PORT));

    }

    private void saveDefaults() {
        propService.saveProp(SCANNER_CONTROLLER_SPEED, speed.getSelectionModel().getSelectedItem());
        propService.saveProp(SCANNER_CONTROLLER_STOP_BIT, stopBit.getSelectionModel().getSelectedItem());
        propService.saveProp(SCANNER_CONTROLLER_BIT, bit.getSelectionModel().getSelectedItem());
        propService.saveProp(SCANNER_CONTROLLER_PORT, port.getSelectionModel().getSelectedItem());
    }
...
//    public void aboutSerial(MouseEvent mouseEvent) throws IOException {
//        FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/about.fxml"));
//        fxmlLoader.setResources(new AboutResource(AboutTextEnum.SERIAL_WHY));
